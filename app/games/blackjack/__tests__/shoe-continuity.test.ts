...
describe("shoe continuity across hands (single seat)", () => {
  test("hand 2 draws cards distinct from hand 1 — the shoe is not rebuilt between hands", () => {
    ...
    act(() => { result.current.placeBet(0, 10); result.current.startRound(); });
    act(() => { result.current.stand(); });
    flush();
    expect(result.current.state.phase).toBe("result");
    const hand1Ids = new Set([
      ...handIds(result.current.state.seats[0].hands[0].cards),
  ...
  test("reshuffle is only ever checked at the next startRound, never mid-round", () => {
    // 18 cards, deckCount=1 (threshold at 13): dealing + a few hits pushes the
    // live remaining count below the threshold WHILE the round is still open.
    ...
    // Player: 4,4=8 → hit,hit,hit → 12,16,20 (never busts, stays playerTurns).
    for (let i = 0; i < 3; i++) {
    ...
    act(() => { result.current.stand(); });
    flush();
    expect(result.current.state.phase).toBe("result"); // resolving the round isn't a reshuffle point either

    act(() => { result.current.resetRound(); });
    expect(result.current.justReshuffled).toBe(false); // still not checked — only startRound checks

    setShoeRanks(Array(60).fill(4)); // what the reshuffle will actually build
    act(() => { result.current.placeBet(0, 10); result.current.startRound(); });
    expect(result.current.justReshuffled).toBe(true); // now, at the round boundary, it fires
  });
